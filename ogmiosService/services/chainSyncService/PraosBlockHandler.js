const { parseMetadata,convertMaestroUtxoAmount, convertMaestroRedeemers, decodeMaestroMetaData } = require("../utilService/commonUtils");

class PraosBlockHandler {
  constructor(mapUtxoManager, bSecurityConfirmEnable, iWanClientObj) {
    this.chainType = "ADA";
    this.blockType = "praos";
    ...
    this.memIWanRecord = new Map();
    this.tokenPairDataValidLatestTs = 600000; //10*60*1000
    this.chainInfoDataValidLatestTs = 3600000; //60*60*1000
  ...
  async getTokenPairInfoById(tokenPairId) {
    ...
    let lastRecord = this.memIWanRecord.get("tokenPairInfo");
    if ((undefined !== lastRecord)
      && ((lastRecord.updatedTs + this.tokenPairDataValidLatestTs) > currentTs)) {
      tokenPairInfo = lastRecord.recordData;
    ...
    try {
      ...
      if (undefined !== tokenPairInfo) {
        let tokenPairRecord = {
          "recordData": tokenPairInfo,
          "recordTs": currentTs
        }
        this.memIWanRecord.set("tokenPairInfo", tokenPairRecord);
      }
  ...
  async getSupportedChainInfo() {
    ...
    let currentTs = Date.now();
    let lastRecord = this.memIWanRecord.get("supportedChainInfo");
    if ((undefined !== lastRecord)
      && ((lastRecord.updatedTs + this.chainInfoDataValidLatestTs) > currentTs)) {
      return lastRecord.recordData;
    }
    ...
    try {
      ...
      let tokenPairRecord = {
        "recordData": mapChainID2Info,
        "recordTs": currentTs
      }
      this.memIWanRecord.set("supportedChainInfo", tokenPairRecord);
  ...
  async parseAddressPrefix(tokenPairId) {
    ...
    let mapChainID2Info = await this.getSupportedChainInfo();
    if (undefined === mapChainID2Info) {
      return undefined;
    }
    ...
    let chainInfo = mapChainID2Info.get(targetChainID);
    ...
    for (let index = 0; index < blockInfo.transactions.length; index++) {
      ...
      if (tx.collateralReturn) {
        ...
        let collateralOutput = {
          ...
          "datumHash": tx.collateralReturn.datumHash
        }
      ...
      if (tx.metadata) {
        ...
        for (let key in metaLabels) {
          ...
          if (undefined !== jsonMetaData.smgID) {
            ...
            if (-1 === jsonMetaData.smgID.indexOf("0x")) {
          ...
          if ((undefined !== jsonMetaData.uniqueId)
            ...
            if (-1 === jsonMetaData.uniqueId.indexOf("0x")) {
              // console.log("\n...parseMetadata...toAccount: ", jsonMetaData.toAccount);
              jsonMetaData.uniqueId = prefix + jsonMetaData.uniqueId;
            }
          }
...
module.exports = PraosBlockHandler;
